...
const moduleFiles=['runtime.js','map-style.js','i18n.js','formatters.js','legacy-localization.js','model.js','traveller.js','traveller-ui.js','ui.js','navigation.js','discovery.js','route-library.js','home.js','regional-shell.js','regional-detail.js','regional-globe.js','regional-timeline.js','regional-controls.js','regional-selection.js','story.js','terrain.js','extensions.js','extensions/cruise.js','extensions/road.js','extensions/border.js'];
...
test('switching to flagship keeps an explicit trip parameter because root is the public home',()=>{
...
test('service worker is network-first so deployed bundles and catalogs cannot remain stale',()=>{
  ...
  assert.match(swSource,/await fetch\(request\)/);
  assert.doesNotMatch(swSource,/caches\.match\(e\.request\)\.then\(hit=>hit\|\|fetch/);
  assert.doesNotMatch(swSource,/data\/platform\/trips\/italy-grand-tour\.json/);
});

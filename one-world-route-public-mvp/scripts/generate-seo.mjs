...
const urls=[base+'/',...route.segments.map(s=>base+'/route/'+s.id+'-'+slug(en(s.from))+'-'+slug(en(s.to))),...route.countries.map(c=>base+'/country/'+slug(en(c.name)))];
const xml='<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n'+urls.map(u=>'  <url><loc>'+u+'</loc></url>').join('\n')+'\n</urlset>\n';
await writeFile(new URL('../sitemap.xml',import.meta.url),xml);console.log('Generated',urls.length,'SEO URLs');
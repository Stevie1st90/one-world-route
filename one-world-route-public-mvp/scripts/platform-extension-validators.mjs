...
export function validatePlatformExtensions(ctx){
  ...
  const cruise=extension(trip,'cruise');
  if(cruise){
    ...
    const cruiseSegments=segs.filter(s=>s.transport?.mode==='cruise'||extension(s,'cruise'));
    ...
    const onboardNights=cruiseSegments.reduce((n,s)=>n+Number(extension(s,'cruise')?.onboardNights||0),0);
    const seaDayNumbers=cruiseSegments.flatMap(s=>extension(s,'cruise')?.seaDayNumbers||[]);
  ...
  const roadTrip=extension(trip,'roadTrip');
  if(roadTrip){
    ...
    const roadSegments=segs.filter(s=>vehicleModes.has(s.transport?.mode)||extension(s,'road'));
    if(!roadSegments.length)fail(item.id+': road-trip extension requires at least one vehicle segment');
    for(const s of roadSegments){
      const road=extension(s,'road');
  ...
  for(const s of segs){
    const border=extension(s,'border');
    if(!border)continue;
  ...
  for(const stop of orderedStops){
    const call=extension(stop,'cruiseCall');
